...
export default async function ClassPresentationPage({
  ...
  return (
    <main className="relative flex min-h-[100svh] items-center overflow-x-hidden bg-[#061a35] px-3 py-4 text-white sm:px-5 md:h-[100svh] md:overflow-hidden md:px-6 md:py-4 lg:px-8 lg:py-5">
      <div
        aria-hidden="true"
        className="absolute -left-40 -top-48 h-[34rem] w-[34rem] rounded-full bg-[#0866ff]/35 blur-3xl"
      />
      <div
        aria-hidden="true"
        className="absolute -bottom-56 -right-32 h-[38rem] w-[38rem] rounded-full bg-[#0099cb]/20 blur-3xl"
      />
      ...
      <div className="relative mx-auto w-full max-w-[1480px] md:flex md:h-full md:flex-col">
        ...
        <article className="grid overflow-hidden rounded-[30px] border border-white/10 bg-white shadow-[0_36px_100px_rgba(0,0,0,0.28)] sm:rounded-[38px] md:min-h-0 md:flex-1 md:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] lg:grid-cols-[minmax(0,1.12fr)_minmax(420px,0.88fr)]">
          <section className="flex min-h-[340px] flex-col overflow-hidden bg-[#0a3569] md:min-h-0">
            <div className="relative min-h-[210px] flex-1 overflow-hidden">
              ...
              <div
                aria-hidden="true"
                className="absolute inset-0 bg-gradient-to-t from-[#071f3f]/35 via-transparent to-black/10"
              />
            </div>

            <div className="shrink-0 bg-[#071f3f] p-5 sm:p-6 md:p-6 lg:p-8">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#8ee3f5] sm:text-sm">
                Heritage Church
              </p>
          ...
          <section className="flex flex-col items-center justify-start bg-[#f8fbff] px-5 py-5 text-center text-[#0b1f3a] sm:px-7 md:min-h-0 md:overflow-y-auto md:px-6 md:py-5 lg:px-8 lg:py-6">
            ...
            <p className="mt-1 max-w-md text-sm leading-5 text-[#5f7187] lg:text-base lg:leading-6">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-[#0754d6] underline decoration-[#0754d6]/30 underline-offset-4 transition hover:decoration-[#0754d6]"
            >
              Open check-in link
            ...
            {presentation.schedule.length ? (
              <div className="mt-3 flex max-h-24 max-w-md items-start gap-2.5 overflow-y-auto rounded-2xl bg-[#eef6ff] px-4 py-2.5 text-left text-xs leading-5 text-[#5f7187] sm:text-sm">
                <Clock3 className="mt-0.5 h-4 w-4 shrink-0 text-[#0866ff]" />
                <div>
                  {presentation.schedule.map((slot) => (
                    <p key={slot.id}>
                      <span className="font-semibold text-[#28415f]">
